...
class LayerAdapter(torch.nn.Module):
    ...
    variational_modeling:
        - False: deterministic adapter
        - True : sample latent correction delta ~ N(mu, sigma^2 I)
    """
    ...
    def __init__(
        ...
        self.variational_use_mean_in_eval = variational_use_mean_in_eval
        self.kl_reduction = kl_reduction
        self.min_std = min_std

        # Exposed after each forward pass for use in the training loss
    ...
    def _compute_variational_delta(self, deterministic_delta, key_padding_mask=None):
        """
        deterministic_delta: (B, T, D)
        returns:
            sampled_or_mean_delta: (B, T, D)
            kl_loss: scalar tensor
        """
        mu = self.var_mean_proj(deterministic_delta)
        logvar = self.var_logvar_proj(deterministic_delta)

        # Optional masking before KL/statistics
        if key_padding_mask is not None:
        ...
        if self.training:
            ...
            delta = mu if self.variational_use_mean_in_eval else mu + std * torch.randn_like(std)

        # KL[q(delta|z) || p(delta)], with p(delta)=N(0, I)
        # elementwise KL = 0.5 * (mu^2 + sigma^2 - 1 - log sigma^2)
        ...
        if key_padding_mask is not None:
            ...
            denom = torch.tensor(kl_per_dim.numel(), device=kl_per_dim.device, dtype=kl_per_dim.dtype)
    ...
    def get_variational_stats(self):
        if not self.variational_modeling:
            raise RuntimeError("Variational modeling is disabled, so variational stats are unavailable.")
        if self.last_mu is None or self.last_std is None:
            raise RuntimeError("No forward pass has been run yet, so variational stats are unavailable.")
